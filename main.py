...
def push_repository(folder_path):
    with Popen(
...
def initialize_local_repo(folder_path):
    ...
    for file in files:
        if file == "README.md":
            ...
            with open(readme_path, "r") as f:
                content = f.read()
                removed_url = remove_url(content).replace(
...
class AutoPyGitHub:
    ...
    def create_repository(self, folder_path):
        ...
        rename_readme(folder_path)

        # Initialize a local Git repository
        initialize_local_repo(folder_path)

        # Add and commit files
        add_and_commit_files(folder_path)

        # Create a main branch
        create_main_branch(folder_path)

        # Add a remote to the local repository
        remote_url = new_repo.clone_url
        add_remote(folder_path, remote_url)

        # Push the local repository to GitHub
        push_repository(folder_path)
    ...
    def main(self):
        ...
        if click.confirm(f"2) Create many repository from : {click.style(repo_name, 'green')}", default=True):
            ...
            if click.confirm(f"3) Create repository located else where", default=True):
                ...
                root_folder = input(
                    click.style(f"Enter folder path \nExample : {click.style(example, 'green')}", "red",
                                bold=True).strip()
                )
